...
export const EditClients = () => {
  ...
  const { name, address, phone, email, isFormValid, onInputChange } = useForm(
    {
      name: state.nombre,
  ...
  const update = async () => {
    if (isFormValid) {
      try {
        ...
        await axios.put(
          `http://localhost:3000/clientes/${params.id}`,
          newClient
        );
        Swal.fire(
          "¡Bien Hecho!",
      ...
      Swal.fire(
        "¡Error!",
        "Todos los campos deben ser rellenados y el teléfono solo debe tener 9 dígitos",
        "error"
      );
    }
  };
